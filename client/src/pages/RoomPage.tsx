  ...
  return (
    <div className="px-4 py-6">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-4">
        ...
        <p className="text-sm text-gray-500 mb-2">你好，{session.nickname}！</p>
        {session.description && (
        ...
        >
          <span>{copied ? '✓ 链接已复制！' : '📤 复制邀请链接'}</span>
        </button>
        <p className="text-xs text-gray-400 text-center mt-2">
          分享给好友，邀请码：<strong>{session.code}</strong>
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        ...
        >
          ...
          <div className="font-semibold text-sm text-gray-800">竞猜</div>
        </button>
        <button
        ...
        >
          <div className="text-2xl mb-1">🏆</div>
          <div className="font-semibold text-sm text-gray-800">排行榜</div>
        </button>
      </div>

      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
        <h3 className="font-semibold text-gray-700 mb-3 text-sm">成员（{members.length}）</h3>
        {members.length === 0 ? (
          <div className="text-sm text-gray-400 text-center py-4">暂无其他成员</div>
        ) : (
            ...
            {members.map((m, i) => (
              <div key={m.id} className="flex items-center gap-3">
                ...
                <span className="flex-1 font-medium text-gray-800">
                  {m.nickname}
                  {m.nickname === session.nickname && <span className="text-xs text-green-500 ml-1">（我）</span>}
                </span>
                <span className="font-bold text-green-600">{m.total_points} 分</span>
              </div>
            ))}
          </div>
        )}
