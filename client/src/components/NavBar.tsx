...
const TABS = [
  { to: '/matches',     icon: '⚽', label: '竞猜'   },
  { to: '/bonus',       icon: '🎯', label: '附加题'  },
  { to: '/leaderboard', icon: '🏆', label: '排行榜'  },
  { to: '/room',        icon: '👥', label: '房间'    },
];
...
export default function NavBar() {
  ...
  function getInviteText() {
    const base = `🏆 加入我的世界杯竞猜房间「${session!.roomName}」！`;
    ...
    <>
      ...
      <header className="bg-green-700 text-white shadow-md">
        <div className="max-w-lg mx-auto px-4 flex items-center justify-between h-14">
          <span className="font-bold text-base flex-shrink-0">⚽ 我猜世界杯</span>
                  ...
                  <div className="absolute right-0 top-12 w-60 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden z-50">
                    ...
                    >
                      ...
                      <div className="text-left">
                        <div className="text-sm font-medium text-gray-800">微信</div>
                        <div className="text-xs text-gray-400">
                          {copiedLink ? '✓ 已复制 — 去微信粘贴！' : '复制消息 → 去微信粘贴'}
                        </div>
                      </div>
                    </button>
                    ...
                    {/* Email */}
                    <a
                      href={`mailto:?subject=${encodeURIComponent('我猜世界杯 邀请')}&body=${encodeURIComponent(getInviteText() + '\n' + getInviteUrl())}`}
                      onClick={() => setShowShare(false)}
                      className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 active:bg-gray-100 transition border-t border-gray-50"
                    >
                      ...
                      <span className="text-sm font-medium text-gray-800">邮件</span>
                    </a>
